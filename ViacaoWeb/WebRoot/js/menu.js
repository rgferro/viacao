...
function menuDropDown(tema) {
   ...
   else  
   {
       ...
       document.styleSheets[0].rules[9].style.backgroundImage = cor_Submenu_NaoAtiva;       
   ...
   for (var i=0; i< navItems.length; i++) {  // Laço de repetiçao que percorre todos o items do menu e muda as propriedades da CSS
      if(navItems[i].className == "submenu") {
         if(navItems[i].getElementsByTagName('ul')[0] != null) {
			...
			navItems[i].getElementsByTagName('ul')[0].style.display="block";navItems[i].style.backgroundImage = cor_Menu_Ativa;
      ...
      if(navItems[i].className == "item") {
         ...
         navItems[i].style.backgroundImage = cor_Submenu_Ativa;
		 navItems[i].style.backgroundImage = cor_Submenu_NaoAtiva;
		 // Alterar a cor de fundo da classe item
         navItems[i].onmouseover=function() {this.style.backgroundImage = cor_Submenu_Ativa;}
         navItems[i].onmouseout=function() {this.style.backgroundImage = cor_Submenu_NaoAtiva;}
